...
const TEMP_MEMBER = {
  ...
  bahano: 30000,           // season advance taken before joining
  fixedSalary: 15000,      // salary for 30 days
  attendedDays: 18,        // days attended in current period
  totalDaysJoined: 180,    // total tenure since joining
  tripsCompleted: 12,      // number of trips completed
  pagar: 9000,             // current month salary (18/30) × 15000
  projectedSalary: 90000,  // projected annual salary (180 days ≈ 6 months)
  upad: 41000,             // total kharchi given
  baaki: -32000,           // pagar - upad
  boat: 'Alpha',
...
// ── Give Kharchi Bottom Sheet Modal ──────────────────────────────────────────
        ...
        <View style={m.sheet}>
          ...
          <View style={m.btnRow}>
            ...
            <TouchableOpacity
              ...
              disabled={!isValid}
            >
              <Text style={m.saveText}>
                Give {amount ? fmt(Number(amount)) : '₹ 0'}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
          ...
          <>
            ...
            <View style={s.calcCard}>
              <View style={s.calcRow}>
                <Text style={s.calcLabel}>Fixed Salary</Text>
                <Text style={s.calcValue}>{fmt(member.fixedSalary)} / 30d</Text>
              </View>
              <View style={s.calcDivider} />
              <View style={s.calcRow}>
                <Text style={s.calcLabel}>Days Attended</Text>
                <Text style={s.calcValue}>{member.attendedDays} days</Text>
              </View>
              <View style={s.calcDivider} />
              <View style={s.calcRow}>
                <Text style={s.calcLabel}>Current Month Salary</Text>
                <Text style={[s.calcValue, { color: theme.colors.primaryDark, fontWeight: '800' }]}>
                  {fmt(member.pagar)}
                </Text>
              </View>
            </View>
            ...
            <View style={s.calcCard}>
              <View style={s.calcRow}>
                <Text style={s.calcLabel}>Trips Completed</Text>
                <Text style={s.calcValue}>{member.tripsCompleted} trips</Text>
              </View>
              <View style={s.calcDivider} />
              <View style={s.calcRow}>
                <Text style={s.calcLabel}>Total Days Joined</Text>
                <Text style={s.calcValue}>{member.totalDaysJoined} days (~{Math.floor(member.totalDaysJoined / 30)} months)</Text>
              </View>
              <View style={s.calcDivider} />
              <View style={s.calcRow}>
                <Text style={s.calcLabel}>Projected Annual Salary</Text>
                <Text style={[s.calcValue, { color: theme.colors.success, fontWeight: '800' }]}>
                  {fmt(member.projectedSalary)}
                </Text>
              </View>
            </View>
            ...
            <View style={s.summaryGrid}>
              <View style={s.summaryCard}>
                ...
                <Text style={s.summaryNum}>180</Text>
                <Text style={s.summarySub}>Days this season</Text>
              </View>
              <View style={s.summaryCard}>
                <Text style={s.summaryEmoji}>🚢</Text>
                <Text style={s.summaryNum}>4</Text>
...
const s = StyleSheet.create({
  ...
  finSub:   { fontSize: 9,  color: theme.colors.textInverse },
  finDivider: { width: 1, backgroundColor: theme.colors.primaryLight, marginVertical: 6 },
  ...
  tabText: { fontSize: 13, fontWeight: '600', color: theme.colors.textPrimary },
  tabTextActive: { color: theme.colors.textInverse },
  countBadge: {
    backgroundColor: 'rgba(0,188,212,0.25)',
    borderRadius: 10, paddingHorizontal: 6, paddingVertical: 1,
  },
  countBadgeText: { fontSize: 10, fontWeight: '700', color: theme.colors.textInverse },

  // Scroll Content
  scrollContent: { padding: 12, gap: 10 },
  sectionTitle: {
    fontSize: 10, fontWeight: '700', color: theme.colors.textPrimary,
    letterSpacing: 1.5, marginTop: 6, marginBottom: 4,
  },
  ...
  calcLabel: { fontSize: 13, color: theme.colors.textSecondary, fontWeight: '500' },
  calcValue: { fontSize: 14, color: theme.colors.textPrimary, fontWeight: '700' },
  calcDivider: { height: 1, backgroundColor: theme.colors.border },
  ...
  summaryNum:   { fontSize: 17, fontWeight: '800', color: theme.colors.textPrimary },
...
const m = StyleSheet.create({
  ...
  input: {
    ...
    padding: 14, color: theme.colors.textPrimary, fontSize: 16, marginBottom: 16,
  },
